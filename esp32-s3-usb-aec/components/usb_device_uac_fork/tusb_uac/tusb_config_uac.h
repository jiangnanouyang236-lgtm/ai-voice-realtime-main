...
// Disable feedback EP - we use Adaptive sync type instead of Asynchronous
...
#define CFG_TUD_AUDIO_FUNC_1_RTOS_TICK_MS         ((1000 + CONFIG_FREERTOS_HZ - 1) / CONFIG_FREERTOS_HZ)
#define CFG_TUD_AUDIO_FUNC_1_MIC_BUF_MS           ((MIC_INTERVAL_MS > CFG_TUD_AUDIO_FUNC_1_RTOS_TICK_MS) ? MIC_INTERVAL_MS : CFG_TUD_AUDIO_FUNC_1_RTOS_TICK_MS)
#define CFG_TUD_AUDIO_FUNC_1_EP_IN_SW_BUF_SZ      CFG_TUD_AUDIO_FUNC_1_FORMAT_1_EP_SZ_IN * (CFG_TUD_AUDIO_FUNC_1_MIC_BUF_MS + 1)
...
// Full-duplex UAC exposes two AS interfaces (speaker + microphone), and TinyUSB uses
// this value to remember their current alternate settings. Using 1 here causes the
// second AS interface state to alias/overwrite the first one when both are present.
