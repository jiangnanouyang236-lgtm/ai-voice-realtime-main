...
typedef enum {
    UAC_STREAM_SPEAKER = 0,
    UAC_STREAM_MICROPHONE,
} uac_stream_t;

typedef void (*uac_stream_state_cb_t)(uac_stream_t stream, bool active, void *cb_ctx);
...
typedef struct  {
    ...
    uac_stream_state_cb_t stream_state_cb;        /*!< callback for host stream interface open/close state */
...
/**
 * @brief Recover speaker and/or microphone streaming after a host-side stall.
 */
