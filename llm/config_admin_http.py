...
class ConfigAdminActions(Protocol):
    ...
    def get_debug_threads(self) -> dict[str, Any]:
        ...
    ...
    def reconnect_mcp_server(self, server_key: str) -> dict[str, Any]:
        ...
...
class _ConfigAdminRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/internal/config/status":
            try:
                ...
                logger.exception("读取配置状态失败: %s", exc)
                self._write_json(
        ...
        if parsed.path == "/internal/debug/threads":
            try:
                payload = self.server.actions.get_debug_threads()
                self._write_json(HTTPStatus.OK, payload)
            except Exception as exc:
                logger.exception("读取线程诊断失败: %s", exc)
                self._write_json(
                    HTTPStatus.INTERNAL_SERVER_ERROR,
                    {"success": False, "message": str(exc)},
                )
    ...
    def do_POST(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path.startswith("/internal/mcp/") and parsed.path.endswith("/reconnect"):
            server_key = parsed.path.removeprefix("/internal/mcp/").removesuffix("/reconnect").strip("/")
            if not server_key:
                self._write_json(HTTPStatus.BAD_REQUEST, {"success": False, "message": "缺少 server_key"})
                return
            try:
                payload = self.server.actions.reconnect_mcp_server(server_key)
                self._write_json(HTTPStatus.OK, payload)
            except Exception as exc:
                logger.exception("重连 MCP 失败: %s", exc)
                self._write_json(
                    HTTPStatus.INTERNAL_SERVER_ERROR,
                    {"success": False, "message": str(exc)},
                )
        ...
        try:
            ...
            logger.exception("重新加载配置失败: %s", exc)
            self._write_json(
                HTTPStatus.INTERNAL_SERVER_ERROR,
                {"success": False, "message": str(exc)},
            )
    ...
    def log_message(self, format: str, *args: Any) -> None:
        logger.info("ConfigAdminHTTP %s - %s", self.address_string(), format % args)
    ...
    def _write_json(self, status: HTTPStatus, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.debug(
                "ConfigAdminHTTP client disconnected before response was sent: %s %s",
                self.command,
                self.path,
            )
...
class ConfigAdminHTTPServer:
    ...
    def start(self) -> None:
        self._thread.start()
        logger.info("内部配置管理 HTTP 已启动: http://%s:%s", self.host, self.port)
